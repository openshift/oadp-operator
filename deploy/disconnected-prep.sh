...
#export OADPVERSION=$(ls deploy/olm-catalog/bundle/manifests/crane-operator.v* | awk -F '.' '{out=""; for(i=2;i<4;i++){out=out$i"."}{out=out$4}; print out}')
export OADPVERSION="oadp-0.4.0"
...
#Get latest images
for i in ${IMAGES[@]}; do
  docker pull quay.io/konveyor/${IMG_MAP[${i}_repo]}:${OADPVERSION} >/dev/null 2>&1
  DOCKER_STAT=$?
  RETRIES=10
...
for i in ${IMAGES[@]}; do
  ...
  while [ -z "${IMG_MAP[${i}_sha]}" ] && [ $RETRIES -gt 0 ]; do
    IMG_MAP[${i}_sha]=$(oc image mirror --dry-run=true quay.io/konveyor/${IMG_MAP[${i}_repo]}:${OADPVERSION}=quay.io/konveyor/${IMG_MAP[${i}_repo]}:${OADPVERSION} 2>&1 | grep -A1 manifests | grep sha256 | awk -F'[: ]' '{ print $8 }')
    let RETRIES=RETRIES-1
  done
...
for f in deploy/olm-catalog/bundle/manifests/oadp-operator.${OADPVERSION}.clusterserviceversion.yaml \
  ...
  if [[ "$f" =~ .*clusterserviceversion.* ]]; then
    ...
    sed -i "s,mig-operator-container:.*,openshift-migration-rhel7-operator:${OADPVERSION},g"                                                                          ${f}
  fi
