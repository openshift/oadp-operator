...
func TestValidateHCP(t *testing.T) {
	...
	originalRequired := RequiredWorkingOperators
	originalAdditional := AdditionalOperators

	// Restore original slices after tests
	defer func() {
		RequiredWorkingOperators = originalRequired
		AdditionalOperators = originalAdditional
	}()
	...
	tests := []struct {
	...
	}{
		{
			...
			deployments: []*appsv1.Deployment{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      RequiredWorkingOperators[0],
						Namespace: HCPNamespace,
					},
				...
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      RequiredWorkingOperators[1],
						Namespace: HCPNamespace,
					},
					Status: appsv1.DeploymentStatus{
						AvailableReplicas: 1,
		...
		},
		{
			...
			expectedError: true,
			errorContains: "required deployment",
		},
		{
			name: "Additional deployment not ready",
			deployments: []*appsv1.Deployment{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      AdditionalOperators[0],
						Namespace: HCPNamespace,
					},
					Status: appsv1.DeploymentStatus{
						AvailableReplicas: 0,
		...
		{
			name: "Unknown deployment not ready",
			deployments: []*appsv1.Deployment{
				{
					ObjectMeta: metav1.ObjectMeta{
		...
		{
			name: "Mix of required and additional deployments",
			deployments: []*appsv1.Deployment{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name:      AdditionalOperators[0],
		...
		{
			name: "Deployment in both required and additional slices",
			deployments: []*appsv1.Deployment{
				{
					ObjectMeta: metav1.ObjectMeta{
			...
			setupTest: func() {
				// Add the same deployment to both slices
				RequiredWorkingOperators = []string{"test-deployment"}
				AdditionalOperators = []string{"test-deployment"}
			},
			expectedError: true,
			errorContains: "cannot be in both RequiredWorkingOperators and AdditionalOperators",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			client := fake.NewClientBuilder().WithScheme(scheme).WithObjects(objectsFromDeployments(tt.deployments)...).Build()
			...
			validateFunc := ValidateHCP(5*time.Second, []string{})
...
// Helper function to convert deployments to runtime.Object
func objectsFromDeployments(deployments []*appsv1.Deployment) []client.Object {
