	...
	return func(ocClient client.Client, _ string) error {
		...
		for _, deployment := range deployments {
			...
			err := wait.PollUntilContextTimeout(context.Background(), time.Second*10, timeout, true, func(ctx context.Context) (bool, error) {
				...
				err := ocClient.Get(ctx, types.NamespacedName{Name: deployment, Namespace: HCPNamespace}, deploymentObj)
				if err != nil {
					if apierrors.IsNotFound(err) {
						log.Printf("\tDeployment %s not found yet, continuing to wait", deployment)
						return false, nil
					}
					return false, err
				}
				...
				log.Printf("\tDeployment %s not ready yet (Available: %d, Replicas: %d)",
					deployment, deploymentObj.Status.AvailableReplicas, deploymentObj.Status.Replicas)
				return false, nil
			})
			if err != nil {
				return fmt.Errorf("deployment %s failed to become ready: %v", deployment, err)
			}
		}
