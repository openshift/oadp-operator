...
func RunMustGather(artifact_dir string, clusterClient client.Client) error {
	// Use MUST_GATHER_IMAGE env var, default to quay.io/konveyor/oadp-must-gather:oadp-1.5
	// For version-specific testing: MUST_GATHER_IMAGE=quay.io/konveyor/oadp-must-gather:oadp-1.4
	mustGatherImage := os.Getenv("MUST_GATHER_IMAGE")
	if mustGatherImage == "" {
		mustGatherImage = "quay.io/konveyor/oadp-must-gather:oadp-1.5"
	}
