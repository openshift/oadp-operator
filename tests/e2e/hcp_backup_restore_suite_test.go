...
func runHCPBackupAndRestore(brCase HCPBackupRestoreCase, updateLastBRcase func(brCase HCPBackupRestoreCase), h *libhcp.HCHandler) {
	...
	err = h.RemoveHCP(libhcp.WaitForObjectDeletionTimeout)
	...
	err = libhcp.ValidateHCP(libhcp.ValidateHCPTimeout, libhcp.ContingencyTimeout, []string{}, h.HCPNamespace)(h.Client, libhcp.ClustersNamespace)
...
var _ = ginkgo.Describe("HCP Backup and Restore tests", ginkgo.Ordered, func() {
	...
	var _ = ginkgo.BeforeAll(func() {
		...
		gomega.Eventually(lib.IsDeploymentReady(h.Client, libhcp.MCENamespace, libhcp.MCEOperatorName), libhcp.WaitForObjectToBePresentTimeout, time.Second*5).Should(gomega.BeTrue())
		...
		gomega.Eventually(lib.IsDeploymentReady(h.Client, libhcp.HONamespace, libhcp.HypershiftOperatorName), libhcp.WaitForObjectToBePresentTimeout, time.Second*5).Should(gomega.BeTrue())
		gomega.Expect(err).ToNot(gomega.HaveOccurred())
	})
	...
	var _ = ginkgo.AfterEach(func(ctx ginkgo.SpecContext) {
		h.RemoveHCP(libhcp.WaitForObjectDeletionTimeout)
	...
	ginkgo.DescribeTable("Basic HCP backup and restore test",
		...
		ginkgo.Entry("None HostedCluster backup and restore", ginkgo.Label("hcp"), HCPBackupRestoreCase{
			...
			BackupRestoreCase: BackupRestoreCase{
				...
				PreBackupVerify:   libhcp.ValidateHCP(libhcp.ValidateHCPTimeout, libhcp.ContingencyTimeout, []string{}, libhcp.GetHCPNamespace(fmt.Sprintf("%s-none", libhcp.HostedClusterPrefix), libhcp.ClustersNamespace)),
				PostRestoreVerify: libhcp.ValidateHCP(libhcp.ValidateHCPTimeout, libhcp.ContingencyTimeout, []string{}, libhcp.GetHCPNamespace(fmt.Sprintf("%s-none", libhcp.HostedClusterPrefix), libhcp.ClustersNamespace)),
		...
		ginkgo.Entry("Agent HostedCluster backup and restore", ginkgo.Label("hcp"), HCPBackupRestoreCase{
			...
			BackupRestoreCase: BackupRestoreCase{
				...
				BackupRestoreType: lib.CSIDataMover,
				PreBackupVerify:   libhcp.ValidateHCP(libhcp.ValidateHCPTimeout, libhcp.ContingencyTimeout, []string{}, libhcp.GetHCPNamespace(fmt.Sprintf("%s-agent", libhcp.HostedClusterPrefix), libhcp.ClustersNamespace)),
				PostRestoreVerify: libhcp.ValidateHCP(libhcp.ValidateHCPTimeout, libhcp.ContingencyTimeout, []string{}, libhcp.GetHCPNamespace(fmt.Sprintf("%s-agent", libhcp.HostedClusterPrefix), libhcp.ClustersNamespace)),
				BackupTimeout:     libhcp.HCPBackupTimeout,
			},
		}, nil),
	)
