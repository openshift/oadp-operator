...
var _ = Describe("Configuration testing for DPA Custom Resource", func() {
	...
	DescribeTable("DPA reconciled to true",
		func(installCase InstallCase) {
			...
			Eventually(dpaCR.IsReconciledTrue(), timeoutMultiplier*time.Minute*2, time.Second*5).Should(BeTrue())
			// TODO do not use Consistently, using because no field in DPA is updated telling when it was last reconciled
			Consistently(dpaCR.IsReconciledTrue(), timeoutMultiplier*time.Minute*1, time.Second*15).Should(BeTrue())
			...
			// Eventually(VeleroPodIsUpdated(kubernetesClientForSuiteRun, namespace, lastInstallTime), timeoutMultiplier*time.Minute*2, time.Second*5).Should(BeTrue())
			...
			Consistently(VeleroPodIsRunning(kubernetesClientForSuiteRun, namespace), timeoutMultiplier*time.Minute*1, time.Second*15).Should(BeTrue())
			...
			if installCase.DpaSpec.Configuration.Restic != nil && *installCase.DpaSpec.Configuration.Restic.Enable {
				...
				Eventually(IsNodeAgentDaemonSetDeleted(kubernetesClientForSuiteRun, namespace), timeoutMultiplier*time.Minute*3, time.Second*5).Should(BeTrue())
			}
			...
			if len(installCase.DpaSpec.Configuration.Velero.PodConfig.Tolerations) > 0 {
				...
				Eventually(VerifyVeleroTolerations(kubernetesClientForSuiteRun, namespace, installCase.DpaSpec.Configuration.Velero.PodConfig.Tolerations), timeoutMultiplier*time.Minute*3, time.Second*5).Should(BeTrue())
			}
			...
			if installCase.DpaSpec.Configuration.Velero.PodConfig.ResourceAllocations.Requests != nil {
				...
				Eventually(VerifyVeleroResourceRequests(kubernetesClientForSuiteRun, namespace, installCase.DpaSpec.Configuration.Velero.PodConfig.ResourceAllocations.Requests), timeoutMultiplier*time.Minute*3, time.Second*5).Should(BeTrue())
			}
			...
			if installCase.DpaSpec.Configuration.Velero.PodConfig.ResourceAllocations.Limits != nil {
				...
				Eventually(VerifyVeleroResourceLimits(kubernetesClientForSuiteRun, namespace, installCase.DpaSpec.Configuration.Velero.PodConfig.ResourceAllocations.Limits), timeoutMultiplier*time.Minute*3, time.Second*5).Should(BeTrue())
			}
	...
	DescribeTable("DPA Deletion test",
		func() {
			...
			Eventually(VeleroIsDeleted(kubernetesClientForSuiteRun, namespace), timeoutMultiplier*time.Minute*3, time.Second*5).Should(BeTrue())
		},
