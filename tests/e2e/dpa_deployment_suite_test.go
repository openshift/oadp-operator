...
var _ = Describe("Configuration testing for DPA Custom Resource", func() {
	...
	DescribeTable("Updating custom resource with new configuration",
		...
		func(installCase InstallCase, expectedErr error) {
			...
			lastInstallingApplicationNamespace = libinit.GetNamespace()
			...
			if installCase.WantError {
				...
				Eventually(dpaCR.DPAReconcileError(), libinit.GetTimeoutMultiplier()*time.Minute*2, time.Second*5).Should(BeTrue())
				return
			}
			...
			Eventually(dpaCR.DoesDPAMatchSpec(libinit.GetNamespace(), installCase.DpaSpec), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
			...
			if len(installCase.DpaSpec.Configuration.Velero.PodConfig.Tolerations) > 0 {
				...
				Eventually(VerifyVeleroTolerations(libinit.GetNamespace(), installCase.DpaSpec.Configuration.Velero.PodConfig.Tolerations), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
			}
			...
			if installCase.DpaSpec.Configuration.Velero.PodConfig.ResourceAllocations.Requests != nil {
				...
				Eventually(VerifyVeleroResourceRequests(libinit.GetNamespace(), installCase.DpaSpec.Configuration.Velero.PodConfig.ResourceAllocations.Requests), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
			}
			...
			if installCase.DpaSpec.Configuration.Velero.PodConfig.ResourceAllocations.Limits != nil {
				...
				Eventually(VerifyVeleroResourceLimits(libinit.GetNamespace(), installCase.DpaSpec.Configuration.Velero.PodConfig.ResourceAllocations.Limits), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
			}
			...
			if installCase.DpaSpec.Configuration.Restic != nil && *installCase.DpaSpec.Configuration.Restic.Enable {
				...
				Eventually(IsResticDaemonsetDeleted(libinit.GetNamespace()), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
			}
			...
			if len(installCase.DpaSpec.Configuration.Velero.DefaultPlugins) > 0 {
				...
				for _, plugin := range installCase.DpaSpec.Configuration.Velero.DefaultPlugins {
					Eventually(DoesPluginExist(libinit.GetNamespace(), plugin), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
				}
			}
			...
			if len(installCase.DpaSpec.Configuration.Velero.CustomPlugins) > 0 {
				...
				for _, plugin := range installCase.DpaSpec.Configuration.Velero.CustomPlugins {
					Eventually(DoesCustomPluginExist(libinit.GetNamespace(), plugin), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
				}
			}
			...
			if installCase.DpaSpec.Configuration.Restic != nil && installCase.DpaSpec.Configuration.Restic.PodConfig != nil {
				for key, value := range installCase.DpaSpec.Configuration.Restic.PodConfig.NodeSelector {
					...
					Eventually(ResticDaemonSetHasNodeSelector(libinit.GetNamespace(), key, value), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
				}
			}
	...
	DescribeTable("DPA Deletion test",
		func(installCase deletionCase) {
			...
			if installCase.WantError {
				...
				Eventually(AreVeleroDeploymentReplicasReady(libinit.GetNamespace()), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).ShouldNot(BeTrue())
				log.Printf("Checking no registry deployment available")
				Eventually(AreRegistryDeploymentsNotAvailable(libinit.GetNamespace()), libinit.GetTimeoutMultiplier()*time.Minute*3, time.Second*5).Should(BeTrue())
			}
		},
