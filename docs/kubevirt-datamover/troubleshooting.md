...
```bash
oc get datauploads.velero.io -n openshift-adp -o custom-columns=NAME:.metadata.name,PHASE:.status.phase,VM:.metadata.annotations.kubevirt-datamover\\.io/vm-name
```
