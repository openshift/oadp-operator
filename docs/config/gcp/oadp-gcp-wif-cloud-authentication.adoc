...
[id="oadp-gcp-console-installation_{context}"]
...
.Console Installation Fields for GCP

During operator installation, the web console will display the following fields:

*GCP Project Number*::
**Field Label:** "GCP Project Number"
+
**Help Text:** "The GCP Project Number required for the operator to access the cloud API."
+
**Value to Enter:** Use the `GCP_PROJECT_NUM` value from the prerequisite setup steps above.

*GCP Pool ID*::
**Field Label:** "GCP Pool ID"
+
**Help Text:** "The GCP Pool ID required for the operator to access the cloud API."
+
**Value to Enter:** Use the `POOL_ID` value from the prerequisite setup steps above (typically matches your `CLUSTER_NAME`).

*GCP Provider ID*::
**Field Label:** "GCP Provider ID"
+
**Help Text:** "The GCP Provider ID required for the operator to access the cloud API."
+
**Value to Enter:** Use the `PROVIDER_ID` value from the prerequisite setup steps above (typically matches your `CLUSTER_NAME`).

*Service Account Email*::
**Field Label:** "Service Account Email"
+
**Help Text:** "The Service Account Email required for the operator to access the cloud API."
+
**Value to Enter:** Use the `SERVICE_ACCOUNT_EMAIL` value from the prerequisite setup steps above (e.g., `velero-your-cluster@your-project.iam.gserviceaccount.com`).

These fields correspond to the Workload Identity Federation configuration and service account you created in the prerequisite steps for Google Workload Identity Federation authentication.

[id="oadp-gcp-cloud-storage-api_{context}"]
== Alternative: Using Cloud Storage API for Automated Bucket Management

Instead of manually creating buckets, you can use the OADP CloudStorage API to automatically manage bucket creation and configuration.
...
.GCP-Specific CloudStorage Configuration

For GCP, create a CloudStorage resource with the following configuration:
...
Verify the GCS bucket was created:

[source,bash]
----
# Check if bucket was created in Google Cloud Storage
