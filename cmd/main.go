...
func main() {
	...
	if common.CCOWorkflow() {
		...
		if credReqCRDExists {
			if common.IsAWSSTS() {
				// Handle AWS STS
				roleARN := os.Getenv(common.RoleARNEnvKey)
			...
			} else if common.IsGCPWIF() {
				// Handle GCP WIF
				audience := os.Getenv(common.AudienceEnvKey)
				serviceAccountEmail := os.Getenv(common.ServiceAccountEmailEnvKey)
