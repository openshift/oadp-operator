...
type ServerFlags struct {
	...
	// How often to ensure all Velero backups in object storage exist as Backup API objects in the cluster. This is the default sync period if none is explicitly specified for a backup storage location. Specify as a Go duration string (e.g. "1h", "30m", "2h30m").
	// +optional
	BackupSyncPeriod *metav1.Duration `json:"backup-sync-period,omitempty"`
	// How long pod volume file system backups/restores should be allowed to run before timing out. Specify as a Go duration string (e.g. "4h", "120m"). (default is 4 hours)
	// +optional
	PodVolumeOperationTimeout *metav1.Duration `json:"fs-backup-timeout,omitempty"`
	// How long to wait on persistent volumes and namespaces to terminate during a restore before timing out. Specify as a Go duration string (e.g. "10m", "1h").
	// +optional
	...
	// How often to check status on backup/restore operations after backup/restore processing. Specify as a Go duration string (e.g. "2m", "30s").
	// +optional
	...
	// How often 'maintain' is run for backup repositories by default. Specify as a Go duration string (e.g. "1h", "30m").
	// +optional
	RepoMaintenanceFrequency *metav1.Duration `json:"default-repo-maintain-frequency,omitempty"`
	// How often garbage collection checks for expired backups. Specify as a Go duration string (e.g. "1h", "30m"). (default is 1 hour)
	// +optional
	...
	// How long to wait on asynchronous BackupItemActions and RestoreItemActions to complete before timing out. Specify as a Go duration string (e.g. "1h", "30m"). (default is 1 hour)
	DefaultItemOperationTimeout *metav1.Duration `json:"default-item-operation-timeout,omitempty"`
	// How long to wait for resource processes which are not covered by other specific timeout parameters. Specify as a Go duration string (e.g. "10m", "1h"). (default is 10 minutes)
