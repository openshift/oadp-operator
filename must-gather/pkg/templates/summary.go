...
func ReplaceBackupsSection(
	...
	volumeSnapshotContentList *volumesnapshotv1.VolumeSnapshotContentList,
	...
	if backupList != nil && len(backupList.Items) != 0 {
		...
		for namespace, backups := range backupsByNamespace {
			...
			for _, backup := range backups {
				...
				var relatedVolumeSnapshotContentList []volumesnapshotv1.VolumeSnapshotContent
				for _, volumeSnapshotContent := range volumeSnapshotContentList.Items {
					if volumeSnapshotContent.Labels[velerov1.BackupNameLabel] == label.GetValidName(backup.Name) {
						relatedVolumeSnapshotContentList = append(relatedVolumeSnapshotContentList, volumeSnapshotContent)
					}
				}
				...
				describeOutput := func(ctx context.Context) string {
					...
					defer cancel()
					return output.DescribeBackup(ctx, clusterClient, &backup, relatedDeleteBackupRequests, relatedPodVolumeBackupLists, relatedVolumeSnapshotContentList, true, skipTLS, "")
				}(context.Background())
