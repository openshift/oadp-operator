...
const (
	mustGatherVersion = "1.3.7"
	mustGatherImage   = "registry.redhat.io/oadp/oadp-mustgather-rhel9:v1.3"
  ...
  oc adm must-gather --image=%[1]s -- /usr/bin/gather --request-timeout 15s --skip-tls`, mustGatherImage),
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if len(importantCSVsByNamespace) != 0 {
				ocAdmInspect := ocadminspect.NewInspectOptions(genericclioptions.NewTestIOStreamsDiscard())
			...
			templates.ReplaceBackupsSection(outputPath, backupList, clusterClient, deleteBackupRequestList, podVolumeBackupList, volumeSnapshotContentList, RequestTimeout, SkipTLS)
